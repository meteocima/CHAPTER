...
- **`hpc/`** - HPC pipeline for Leonardo (CINECA). Uses Hydra config (`conf/pipeline.yaml`) with CLI overrides.
  ...
  - **Output tree is `grib_v2/`** (GRIB2, 231 messages), not the old `grib/` (GRIB1, 93): re-entrancy keys on the output GRIB existing, so writing the new schema into the old tree would silently skip every hour already converted
...
- A CHAPTER wrfout is ~9.14 GB; one timestep converts to a **~710 MB GRIB2 (231 messages, 13 pressure levels) in ~5 min on 1 DCGP core**, peak RSS **19.0 GB** (hence `slurm.step_convert_mem: 32G`). Measured 2026-09-17: 710 MB for a summer afternoon at 231 messages (the same file was 761 MB at 246, before the soil and vegetation blocks were dropped), so ~17 GB/day and **~6,4 TB/year**. The earlier 182-message schema was 663 MB in ~2:47. It is still *faster* than the old 93-message GRIB1 (~7.5 min): `grid_ccsds` packs both smaller and much faster than `grid_second_order`.
...
- **Variable schema**: 75 entries -> 231 messages, defined once in `wrf_era5_comparison.WRF_TO_ECMWF_PARAMID` (13 variables x 13 pressure levels + 62 single-level). `EXPECTED_MESSAGES` comes from it and `hpc/check_grib_sanity.py` enforces it, so a field that silently fails to compute is caught. The 13 pressure-level variables are the ERA5 set plus both vertical-velocity conventions (`w` in Pa/s, `wz` in m/s) and the three hydrometeor/cloud fields `cc`/`crwc`/`cswc`
...
- **Deliberately dropped, not approximated** (2026-09-17): soil moisture and temperature (`swvl1-4`, `stl1-4`) because RUC's point values at 0/5/20/40 cm are not ERA5's 0-7/7-28/28-100/100-289 cm layer averages; `tvl`/`tvh`/`slt` because WRF's MODIS-IGBP and STATSGO category numbers are not ECMWF's code table 4.234; `cvl`/`cvh`/`lai_lv`/`lai_hv` because they all assume one dominant vegetation category per cell while ERA5 lets high and low coexist, and `VEGFRA` is a seasonal green fraction, not a cover fraction. The rule that came out of it: a mapping is published only if the WRF quantity IS the ERA5 one, or the difference can be stated in one line
