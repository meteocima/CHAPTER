...
- **`hpc/`** - HPC pipeline for Leonardo (CINECA). Uses Hydra config (`conf/pipeline.yaml`) with CLI overrides.
  ...
  - **Output tree is `grib_v2/`** (GRIB2, 182 messages), not the old `grib/` (GRIB1, 93): re-entrancy keys on the output GRIB existing, so writing the new schema into the old tree would silently skip every hour already converted
...
- A CHAPTER wrfout is ~9.14 GB; one timestep converts to a **~663 MB GRIB2 (182 messages, 13 pressure levels) in ~3 min on 1 DCGP core**, peak RSS **18.2 GB** (hence `slurm.step_convert_mem: 32G`). Measured 2026-09-16 on 2024-07-01T14. It is *faster* than the old 93-message GRIB1 (~7.5 min) despite twice the messages: `grid_ccsds` packs both smaller and much faster than `grid_second_order`.
...
- **Variable schema**: 62 entries -> 182 messages, defined once in `wrf_era5_comparison.WRF_TO_ECMWF_PARAMID` (10 variables x 13 pressure levels + 52 single-level). `EXPECTED_MESSAGES` comes from it and `hpc/check_grib_sanity.py` enforces it, so a field that silently fails to compute is caught
- **What the run cannot give**, whatever we do to the converter: no TKE and no PBLH (`BL_PBL_PHYSICS=1`, YSU is non-local, `KM_OPT=4`), no ocean/wave coupling (`SF_OCEAN_PHYSICS=0`) so no waves/currents/SSH/ice thickness, no `TSK`, `HFX`, `LH`, `ALBEDO`, no `LANDUSEF`/`GREENFRAC` (those live in `geo_em_d02`, not in the wrfout). `ACHFX`/`ACLHF` exist but are identically zero. Full reasoning and the possible workarounds: `MISSING_VARIABLES.md`
- **Fields present but dead** (identically zero on every sampled timestep, 2019 + 2024 + 2024-03): `ACHFX`, `ACLHF`, `ACGRDFLX`, `NOAHRES`, `SSTSK`, `SST_INPUT`, `SWNORM`, `ACSNOW`, `HAILNC`, `RAINC`, `RAINSH`, `PREC_ACC_C`, `LWDNT`/`LWDNTC`/`ACLWDNT`/`ACLWDNTC`. `ACSNOM` is alive but **not monotone** (points reset within a run), so it is not published as `smlt`
- **`CLDFRA` is binary 0/1** (`ICLOUD=1`), so `tcc`/`hcc`/`mcc`/`lcc` are maximum-random overlaps of a binary field. `SST` is constant within each 24 h run (`SST_UPDATE=0`). `skt` is derived from `LWUPB` because `TSK` was not written out
