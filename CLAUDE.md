...
- **`convert_to_pressure_levels.py`** - Main conversion script: reads WRF NetCDF, computes diagnostics, encodes GRIB2 with projection metadata. Accepts `--input`/`--output` CLI args. **What is produced and on which level lives entirely in the registry** (`wrf_era5_comparison.WRF_TO_ECMWF_PARAMID`); this module only knows how to compute each field and hand it to eccodes, so adding a variable is one row of the registry plus (if derived) one block
- **`static_ref.py`** - The geo_em sidecar: reads `geo_em.d02` once and writes `static_v1/chapter_static_d02.npz` (1.8 MB) with `cvl`/`cvh`/`tvl`/`tvh`/`slt`/`cl`/`dl`. **Every mapping decision lives here**, not in the converter, so the whole crosswalk sits in one auditable place and the sidecar meta records the table that produced the numbers. `python static_ref.py extract <geo_em.d02.nc> <static_dir> --check-wrfout <wrfout> ...`, where `--check-wrfout` tests the assumption (identical grid, `LU_INDEX`==`IVGTYP` bar the lake recode) and refuses to write if the grids differ
- **`accum_ref.py`** - 00Z reference sidecars for accumulated fields (tp referred to 00Z of the same day); `python accum_ref.py extract <wrfout_00Z> <accum_ref_dir>`
...
- **`hpc/`** - HPC pipeline for Leonardo (CINECA). Uses Hydra config (`conf/pipeline.yaml`) with CLI overrides.
  ...
  - **Output tree is `grib_v3/`** (GRIB2, 246 messages); before it `grib_v2/` (GRIB2, 231) and `grib/` (GRIB1, 93). A new tree at every schema change because re-entrancy keyed on the output GRIB merely existing, so a wider schema written into the old tree silently skipped every hour already converted. **Fixed at the source on 2026-09-18**: `convert_step.sh`/`convert_day.sh` now skip only when the existing file holds `EXPECTED_MESSAGES` messages (`hpc/grib_schema_ok.py`, dependency-free header walk, ~0.3 s), so a re-run across a schema change is self-healing and the next change needs no new tree
...
- **The converter is live code too, and the whole repo is read from every compute node.** Editing `convert_to_pressure_levels.py` or `wrf_era5_comparison.py` while convert jobs are running made 199 of them die with `SyntaxError: source code string cannot contain null bytes` on the import — the shared HOME filesystem served a stale/partially-written view to nodes that had the file cached. It started with the first batch after the edit and faded out over ~6 hours, ~4 jobs per batch, then stopped by itself; the files on disk were never corrupt. The failures are not silent (no GRIB is written, the wrfout is kept) but they leave holes that only a re-run fills. **Do not edit the repo while a campaign is running**; if you must, stop the chains, edit, then restart. Unlike `fetch_step.sh`, the converter is not snapshotted — the convert job runs it straight from `$PROJECT_DIR`.
...
- **eccodes runtime:** the convert jobs must `module load eccodes/2.34.0--gcc--12.2.0` and set `LD_PRELOAD`/`LD_LIBRARY_PATH` to the gcc-12 runtime + eccodes lib64; the python eccodes bindings fail to find the C lib without this env. The `grib_*` CLI tools are not merely off PATH, they are **absent** from this spack install (there is no `bin/` in the eccodes prefix) — which is why `hpc/grib_schema_ok.py` counts messages by walking the GRIB headers itself instead of calling `grib_count`. `source tools/eccodes_env.sh` does exactly this for interactive use (`hpc/convert_step.sh` inlines the same paths).
...
- A CHAPTER wrfout is ~9.14 GB; one timestep converts to a **~747 MB GRIB2 (246 messages, 13 pressure levels) in ~6 min on 1 DCGP core**, peak RSS **19.9 GB** (hence `slurm.step_convert_mem: 32G`). Measured 2026-09-18 on three real files: 749.6 MB for a summer afternoon, 650.3 MB at 00Z, 772.4 MB for a March midday, so ~17.9 GB/day and **~6,55 TB/year**. The 15 fields geo_em unlocked cost **+39.2 MB per file, flat** (seven are constant in time, the eight soil layers barely vary) — measured message by message, and confirmed to the byte by the full conversion. The same file was 710.4 MB at 231 messages and 663 MB at 182 in ~2:47. It is still *faster* than the old 93-message GRIB1 (~7.5 min): `grid_ccsds` packs both smaller and much faster than `grid_second_order`.
...
- **`CLDFRA` is fractional, not binary.** `ICLOUD=1` is the Xu-Randall scheme: measured histogram over a 500x600 subdomain, only 18-22% of cloudy points sit exactly at 1 and the rest spread over the whole 0-1 range (614k distinct values). An earlier note in this file claimed it was binary — it was wrong. So `tcc`/`hcc`/`mcc`/`lcc` are honest maximum-random overlaps and `cc` on pressure levels is a legitimate ERA5 field. `SST` is constant within each 24 h run (`SST_UPDATE=0`). `skt` is inverted from `LWUPB` because `TSK` was not written out: `LWUPB = eps*sigma*T^4 + (1-eps)*GLW`, with eps the per-category emissivity MEASURED from the run (`WRF_EMISS`, see below), not a table lookup. Both the reflected term and the per-category eps matter: over open water, where the model's skin temperature IS the SST, this recovers it to 0.0015 K, against +1.25 K for a fixed 0.98 with no reflection
- **`geo_em.d02` IS recoverable, and was recovered** (2026-09-18, from LRZ; it sits in `/leonardo_work/AIFPT_AILAMIT/CHAPTER/geo_em/`, out of HOME and out of the git worktree). It is the run's own static file: `XLAT_M`/`XLONG_M` match a wrfout to `0.0`, and `LU_INDEX` agrees with `IVGTYP` on **100.00%** of points once the lake recode is excluded (`sf_lake_physics=0` turns class 21 into 17 — which is why `cl` is information `lsm` does not carry: 19309 land cells hold a sub-grid lake). It carries the FRACTIONAL cover the hourly output reduced to one dominant category: that category holds only 90.1% of a land cell on average, and **17.9% of land cells carry both high and low vegetation above 5%**. Read once by `static_ref.py` into `static_v1/chapter_static_d02.npz` (1.8 MB) — never per timestep. The converter loads it outside any try/except and checks the grid fingerprint, so a geo_em from another domain aborts instead of poisoning the archive
- **Published after all** (2026-09-18), each because the WRF quantity IS the ERA5 one: `cvl`/`cvh` (sums of `LANDUSEF` over the low/high MODIS classes); `tvl`/`tvh` (dominant class translated to code table 4.234, every row a semantic identity — `tvl` is **masked** on the 124672 cells where a shrubland dominates, because MODIS carries no shrub phenology while 4.234 splits evergreen from deciduous, and guessing was refused); `slt` (the FAO clay/sand thresholds that *define* ECMWF's 7 types, applied to `CLAYFRAC`/`SANDFRAC` — no category crosswalk); `cl`/`dl` (`dl` masked off-lake: `LAKE_DEPTH` is the 10 m WPS default on 99.56% of the domain); `swvl1-4`/`stl1-4` as **exact ERA5 layer averages** — RUC has 6 nodes at 0/5/20/40/160/300 cm and ERA5's deepest boundary (289 cm) falls inside the deepest node, so each layer is the exact integral of RUC's own piecewise-linear profile and nothing is extrapolated. The rule still holds: a mapping is published only if the WRF quantity IS the ERA5 one, or the difference can be stated in one line
- **Still not published**: `lai_lv`/`lai_hv` (the model carries one LAI per cell, ERA5 wants one per class; any split is an assumption) and `anor`/`isor` (`OA1-4`/`OL1-4` are Kim-Arakawa asymmetry and effective length, not ECMWF's gradient-covariance eigenstructure)
- **`skt`: the emissivity was MEASURED, not looked up** (2026-09-18). RRTMG's clear-sky diagnostic shares the all-sky equation's `eps` and `T` and differs only in the downward flux, so the two together eliminate the temperature: `eps = 1 - (LWUPB-LWUPBC)/(LWDNB-LWDNBC)`. An identity, no reference temperature, no fit. It returns **IQR 0.0000** within every land-use category and **0.98000 exactly** over water where the answer is known independently. Ten of the fourteen categories present equal VEGPARM's `EMISSMIN`; **four do not, and RUC takes them from neither VEGPARM.TBL nor LANDUSE.TBL**: cat 3 and 5 are 0.940, cat 7 is 0.880, cat 12 is 0.935, cat 15 is 0.980, and cat 16 (barren, a quarter of the domain by area) is **0.850 against EMISSMIN's 0.900**. Snow switches every category to a flat 0.98, applied from `SNOWC >= 0.01` (97.96% exact, against 95.32% for a pure blend on snow cover) — so the old claim that the emissivity under snow could not be reconstructed is wrong. Result: the inversion reproduces the model's own emissivity to 1e-4 on **97.96% of cloudy land points against 14.78% before**, skt RMSE **0.277 K -> 0.032 K**, water gate untouched at 0.0015 K. `WRF_EMISS` in the converter holds the measured table; the five categories that never occur in this domain keep `EMISSMIN` and are marked NOT MEASURED.
- **Three `skt` hypotheses were refuted, all by measurement — do not re-propose them**: a `LANDUSEF`-weighted mix (breaks the water gate, 0.0015 K -> 0.98 K; with `sf_surface_physics=3` WRF never sees `LANDUSEF` at runtime); `EMISSMIN + shdfac*(EMISSMAX-EMISSMIN)`, WRF's own init formula (the measured eps is **flat** against VEGFRA where that demands +0.04..0.065 — grass in July measures 0.9200 against the summer table's 0.96); and `EMISSMIN` itself on those four categories. Note also that `SOILT1` is **not** a skin reference — it is the temperature inside snow; the right soil comparison is `TSLB[0]`.
- **WRF's `Q*` are mixing ratios** (per kg of dry air); `q`, `clwc`/`ciwc`/`crwc`/`cswc` and every column integral are specific contents (per kg of moist air), so they are divided by `1 + total water mixing ratio` -- **the same denominator for all of them**, because moist air includes the condensate. Measured on the full 3D field: **2.26% at most, 1.44% at p99, 0.011% in the median**, always in excess. Quote the percentile, not the median: most of a 49-level column is dry upper troposphere, so the median describes the population and not the correction. Until 2026-09-25 `q` alone divided by `1 + QVAPOR`, i.e. two definitions of moist air in one file, up to 1.13% apart (issue #41)
- **`r` and `2r` are ECMWF's relative humidity, not wrf-python's** (2026-09-25, issue #40). `getvar('rh')`/`getvar('rh2')` call `fortran/wrf_user.f90:722`, which saturates over **liquid water at every temperature** and then applies `MAX(MIN(qv/qvs,1),0)`. ECMWF's paramId 157 saturates over the **mixed phase** — all ice below 250.16 K, all water above 273.16 K, **quadratic** in the liquid fraction between — and **does not clip**, because supersaturation is physical and ERA5 publishes it. Over ice the two differ by up to a factor **1.8**: the old field was **0.505** of ERA5 at 100 hPa and the corrected one is 0.945 to 1.058 at every level. The definition is `ifs_humidity.py` at the repository root, every constant quoted from IFS Cy41r2 (the cycle ERA5 was produced with) and checked by its own `self_check()`. **The converter computes `r` and `2r` there; do not go back to `getvar`.** Expect values above 100: up to 141.9% above ground in the ice regime, and up to 163.1% below ground — the latter is the clamped extrapolation of #44 pairing ~25 m AGL moisture with a 1000 hPa nominal, not a defect in the humidity
- **The land/sea mask is NOT constant in time.** WRF reclassifies points with `SEAICE>0` as ice: `LANDMASK` 0->1, `IVGTYP` 17->15, `ISLTYP` 14->16, and `ALBBCK`/`SNOALB` change with them (measured: 2469 points on 2024-03-20, exactly the sea-ice points; identical fields between 2019-06 and 2024-07). `lsm` and `al` are therefore written in every message and must not be treated as one-per-archive statics. **The soil columns are no longer in that list either**: since 2026-09-25 `swvl1-4`/`stl1-4` use geo_em's permanent land mask, because the columns WRF grows under new sea ice are not soil — `swvl` reached exactly 1.000 against soil type 16's porosity of 0.435 (#38). `tvl`, `tvh` and `slt` are **not** in that list any more: since the geo_em recovery they are computed once by `static_ref.py` and read from the static sidecar, so they never see the model's hourly mask at all and are invariant across the whole archive. (Their own masks are unrelated to sea ice: `slt` is NaN off geo_em's permanent land mask, `tvl`/`tvh` are 0 where that cover is absent and `tvl` is NaN on shrubland)
- **One masking rule for the whole archive** (2026-09-25, issues #34/#36/#38): **a point is masked where the ERA5 parameter is not defined there** — not where the model has nothing to say, and not where the number looks wrong. `sst` and `ci` on the **sea**; `cl`/`dl` zero and missing respectively on the sea; `swvl1-4`/`stl1-4` on geo_em's **permanent** land, so sea ice carries no soil column; masked points are a GRIB **bitmap**, never zeros. `lsm` and `al` still follow the model hour by hour, because that is what it integrated
- **The sea/inland-water discriminator lives in the static sidecar, and it is NOT geo_em's lake class.** WPS's MODIS class 21 calls the Black Sea, the Sea of Azov and part of the Baltic a lake — 89% of the cells this domain marks fully lake — so masking on it would have removed `sst` from the Black Sea. Nor is it "the component touching the domain edge": the Bosporus is ~700 m against a 3 km grid, so the Black Sea is **not** connected to the Mediterranean here, while Lake Ladoga **is** clipped by the northern boundary. What works is connected-component **size**: measured, the water components are 844431 (Atlantic+Med+Baltic+North Sea), 49841 (Black Sea+Azov), 8583 (Red Sea), then 1225 (Vänern, the largest inland lake) and smaller. `static_ref.OCEAN_MIN_CELLS = 2000` sits in a gap with a factor of **seven** of slack — checked at 1226/2000/4000/8583, all giving 902855 ocean cells — and `static_ref.py` prints the margin at every extraction so a different geo_em cannot reclassify a sea silently
...
- **`10fg` is the one non-00Z accumulation, and its declared window is right only 88% of the time.** `WSPD10MAX` is reset by WRF at the history write, so it is encoded with reference time H-1, step `0-1`, `stepType=max` — the encoding itself was verified correct. **The field is not always a 1-hour maximum**: measured over six whole days, 138 transitions, the reset was missed at **17 of them (12.3%)**, and those hours carry a maximum over two hours or more. Detecting it needs the PREVIOUS hour's wrfout, which in backward order is not on disk when the convert runs, and there is no intra-file detector — all seven `nwp_diagnostics` maxima reset on the same alarm, so when it is missed they all carry over together and none can witness for another. **Decided 2026-09-25: left as `0-1` and documented** (issue #49); making the window exact means the driver must fetch H-1 before submitting H's convert, which is tracked separately and is not a blocker. Mean excess over the instantaneous 10 m wind is 0.068-0.311 m/s on hours that reset and **0.499-0.784 on hours that did not**. It is the max of the *resolved* 10 m wind, not a gust parameterisation, and it runs at 0.625 of ERA5's `i10fg` (#50)
- **00Z reference sidecar** (`accum_ref.py`): `<work_dir>/accum_ref_v2/YYYY/MM/accum_ref_YYYYMMDD.npz`, native WRF units. **Adding a name to `ACCUMULATED_VARS` makes every existing sidecar incomplete** (`load_sidecar` raises `KeyError`), so those days must be re-extracted from their 00Z wrfout. The 246-message schema needs **19** accumulators (precipitation, runoff and the full radiation set, all verified monotone within a run), against the 12 of the previous schema and the 6 the sidecars on disk actually carry — which is why the tree is `accum_ref_v2` and the old `accum_ref` is left alone for `grib/` and `fix_tp_accum.py`. Beware that `ensure_ref` trusts any non-empty sidecar it finds without checking its contents. Sidecar contents: written atomically by the converter for 00Z input, by `fetch_step.sh` `ensure_ref` (which fetches the 00Z first when needed — in backward order it would arrive last) and by `fix_tp_accum.py` (from the old 00Z GRIB). Invariant: the 00Z wrfout is never deleted without its sidecar. A convert with no reference fails loudly (no GRIB) rather than writing run-init-referred tp
