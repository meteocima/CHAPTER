...
- **The geo_em does NOT improve `skt`, it makes it worse.** Weighting the emissivity by `LANDUSEF` breaks the one gate where the answer is known exactly: over open water `max|skt-SST|` goes from **0.0015 K to 0.98 K**, because a mixed coastal cell's weighted emissivity is not water's. With `sf_surface_physics=3` WRF never sees `LANDUSEF` at runtime — it looks the emissivity up by the DOMINANT `IVGTYP`, so a "more realistic" emissivity is *less* faithful to the inversion. `skt` is unchanged. The improvement that does exist needs no geo_em: `EMISSMIN + shdfac*(EMISSMAX-EMISSMIN)` with `shdfac = VEGFRA/100`, WRF's own initialisation formula, which beat the current one in all four dominant-fraction bins (7.19 vs 7.37 K against `SOILT1`) without touching the water gate. Not adopted yet: it needs measuring at night, where `SOILT1` is actually comparable to the skin (at 14Z the +6 K bias is physics, not error)
