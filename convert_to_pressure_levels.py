...
hour="11"
# Configuration
input_file = f"/home/nloglisci/2023032718/wrfout_d02_2023-03-28_{hour}:00:00"
# Output file following Anemoi naming convention:
# Format: purpose-content-source-resolution-start-year-end-year-frequency-version
output_file = f"/home/lmonaco/CHAPTER/output/ailam-an-cima-3km-2023-2023-1h-v1-20230328{hour}.grib"
...
# DEBUG: limit processing to these variables only (leave empty [] to process all)
# Examples: ['T2', 'U10', 'V10'] for surface, ['tk', 'rh'] for 3D, ['slor'] for single debug
DEBUG_VARS = []  # [] = all variables, or list e.g. ['T2', 'tk', 'slor']
...
try:
    ...
    print(f"⚠️  Unable to load LANDMASK: {e}")
...
if 'tcw' in WRF_TO_ECMWF_PARAMID and (not DEBUG_VARS or 'tcw' in DEBUG_VARS):
    ...
    try:
        ...
        # Vertical integration: TCW = ∫ q_total * (dp/g)
        g = 9.81  # m/s^2
...
if not output_vars:
    print("\n⚠️  ERROR: No variables to write to GRIB file!")
...
if os.path.exists(output_file):
    ...
    if file_size == 0:
        print("⚠️  WARNING: GRIB file is empty!")
    if written_count == 0:
        print("⚠️  WARNING: No GRIB messages written!")
else:
    print(f"⚠️  ERROR: File {output_file} was not created!")
